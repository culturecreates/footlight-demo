...
d3sparql.fetch = function(url, callback) {
  ...
  d3.xhr(url, mime, function(request) {
    ...
    search_button.classList.remove("is-loading")
  })
/*
...
d3sparql.tree = function(json, config) {
  ...
  var pair = d3.map()
  if (d3sparql.debug) { console.log("pair",  pair) }
  var size = d3.map()
  if (d3sparql.debug) { console.log("size", size) }
  var root = data[0][opts.root].value
  if (d3sparql.debug) { console.log("root", root) }
  var parent = child = children = true
  for (var i = 0; i < data.length; i++) {
    parent = data[i][opts.parent].value
    child = data[i][opts.child].value
    if (d3sparql.debug) {console.log("parent/cild",parent,child)}
    if (parent != child) {
      if (pair.has(parent)) {
  ...
  function traverse(node) {
    if (d3sparql.debug) { console.log("traverse - node",node) }
    var list = pair.get(node)
    if (d3sparql.debug) { console.log("traverse - list",list) }
    if (list) {
